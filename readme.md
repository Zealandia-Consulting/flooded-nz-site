...
This page is one HTML file with no backend and no build step, so it doesn't need a full cloud hosting platform, just somewhere to serve a file over HTTPS with a custom domain. A few options were weighed before settling on GitHub Pages.

### Options considered

**GitHub Pages (chosen):** free, with no separate cloud account to manage; the repo that holds the code is the same place that serves it. Publishing is a plain `git push`, and GitHub notices and serves the new version within a minute or two. Free custom domain with a free, auto-renewing HTTPS certificate, both built in.
...
- a GitHub Actions integration, which is more moving parts: a deployment token stored as a secret, plus a workflow file wiring GitHub and Azure together, or
